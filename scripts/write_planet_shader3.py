"""
Writes lib/planetShader.ts — improved version with uVariant uniform
for more visual variety. Builds on the lean WebGL-safe shader.
"""
import pathlib
...
FRAG = r"""
  ...
  uniform float uSeed;
  uniform float uTime;
  uniform float uVariant;  // 0..1 for sub-type variation
  varying vec3 vPos;
  varying vec3 vWorldPos;
  varying vec3 vWorldNorm;
...
""" + SNOISE_GLSL + r"""
  ...
  void main(){
    ...
    float bStr = (uType > 1.5) ? 0.08 : 0.40;
    vec3 bumpN = normalize(vWorldNorm + grad*bStr);
    ...
    // ════════════════════════════════════════════════════════════
    //  ROCKY  (uType < 0.5)  —  3 palette variants
    // ════════════════════════════════════════════════════════════
    if (uType < 0.5) {
      ...
      vec3 hiC, loC;
      if (uVariant < 0.33) {
        // Mercury/Moon — grey/tan
        hiC = hsv(vec3(0.08+uHue*0.06, 0.18, 0.62));
        loC = hsv(vec3(0.07+uHue*0.04, 0.12, 0.28));
      } else if (uVariant < 0.66) {
        // Mars-like — rusty red/ochre
        hiC = hsv(vec3(0.04+uHue*0.05, 0.65, 0.58));
        loC = hsv(vec3(0.02+uHue*0.04, 0.55, 0.22));
      } else {
        // Io-like — sulphur yellow/orange
        hiC = hsv(vec3(0.12+uHue*0.08, 0.78, 0.68));
        loC = hsv(vec3(0.08+uHue*0.06, 0.60, 0.30));
      }
    ...
    // ════════════════════════════════════════════════════════════
    //  TERRESTRIAL  (0.5 <= uType < 1.5) — ocean/land variation
    // ════════════════════════════════════════════════════════════
    } else if (uType < 1.5) {
      float landN = fbm(p*2.0+seed3)*0.65 + fbm(p*5.5+seed3.zxy)*0.35;

      // Vary ocean ratio: uVariant shifts the threshold
      float seaLevel = 0.42 + uVariant * 0.22;  // 0.42 (mostly ocean) to 0.64 (mostly land)
      oceanMask   = 1.0 - smoothstep(seaLevel - 0.07, seaLevel + 0.07, landN);
      ...
      if (uVariant < 0.40) {
        // Earth-like green
        jungle = hsv(vec3(0.31+uHue*0.06, 0.72, 0.30));
        desert = hsv(vec3(0.10+uHue*0.05, 0.52, 0.64));
      } else if (uVariant < 0.70) {
        // Alien-lush — teal/purple vegetation
        jungle = hsv(vec3(0.48+uHue*0.08, 0.65, 0.34));
        desert = hsv(vec3(0.06+uHue*0.04, 0.42, 0.58));
      } else {
        // Arid world — mostly desert/savanna
        jungle = hsv(vec3(0.22+uHue*0.05, 0.45, 0.38));
        desert = hsv(vec3(0.08+uHue*0.06, 0.58, 0.70));
      }
      ...
      cloudAlpha = smoothstep(0.48, 0.68, cl) * 0.85;
    ...
    // ════════════════════════════════════════════════════════════
    //  ICE GIANT  — Neptune/Uranus variants
    // ════════════════════════════════════════════════════════════
    } else if (uType < 2.5) {
      float warp = fbm(p*1.5+seed3) * 0.18;
      float dLat = lat + warp;

      // Vary band count: 5–14 bands depending on variant
      ...
      // Colour: deep blue (Neptune) vs teal (Uranus) by variant
      float hB  = mix(0.57, 0.48, uVariant) + uHue*0.11;
      float satB = mix(0.90, 0.55, uVariant);  // Uranus-types more pastel
    ...
    // ════════════════════════════════════════════════════════════
    //  GAS GIANT  — Jupiter-like with metallic shimmer
    // ════════════════════════════════════════════════════════════
    } else {
      float wind = fbm(p*1.5+seed3)*0.26 + fbm(p*4.0+seed3.yzx)*0.10;
      float dLat = lat+wind;

      // More bands for larger variant values
      ...
      // White oval storms (secondary) — analytic position
      ...
      // Colour palette — vary from warm beige/ochre to cool grey/blue
      ...
      atmosCol = hsv(vec3(fract(0.09+hShift), 0.50, 0.98));
      atmosStr = 0.35;
      specPow  = 24.0;
    }
    ...
    // ── Polar aurora (terrestrial + ice giant) ─────────────────
    if (uType >= 0.5 && uType < 2.5) {
      ...
      vec3 aCol    = (uType < 1.5)
                   ? mix(vec3(0.1,0.9,0.3), vec3(0.3,0.2,0.9), aWave)
 ...
 * all variables initialized, float uniforms throughout.
 * Includes uVariant uniform for sub-type visual variety.
 */
...
export function createPlanetMaterial(
  ...
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uSeed:    { value: seed },
      uTime:    { value: 0    },
      uVariant: { value: seed },  // seed is already 0..1, reuse as variant
    },
  });
}
